...
export default function AnonymousEntryPage() {
  ...
  const handleEnter = async () => {
    try {
      setIsEntering(true);
      setError('');
      await loginAnonymous(identity.displayName, AVATARS[selectedAvatar].src);
      router.push('/dashboard');
    } catch (loginError) {
      setError(loginError.message || 'Unable to enter your safe space right now.');
      setIsEntering(false);
    }
  };
    ...
    <div className={styles.page}>
      <div className={styles.card}>
        ...
        <div className={styles.promises}>
          {['No email or phone', 'Anonymous identity backed up securely', 'Leave anytime'].map((p, i) => (
